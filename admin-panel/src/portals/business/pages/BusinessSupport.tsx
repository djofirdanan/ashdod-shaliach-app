...
const BusinessSupport: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    addSupportMessage(ticket.id, 'user', biz?.businessName ?? 'עסק', text.trim());
    playNewMessage();
    setText('');
    ...
    <div className="max-w-lg mx-auto flex flex-col h-[calc(100vh-130px)]">
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-[11px]" style={{ color: '#8898aa' }}>צוות אשדוד-שליח כאן לעזור</p>
        </div>
        {ticket && (
          <span
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{ background: '#f6f9fc' }}>
        {messages.length === 0 && (
          <div className="text-center mt-10">
            ...
            <p className="text-[14px] font-bold mb-1" style={{ color: '#061b31' }}>יש שאלה? אנחנו כאן</p>
